...
export const leaderboardRouter = router({
  ...
  saveScore: publicProcedure
    ...
    .mutation(async (opts) => {
      ...
      const result = await db.query(`INSERT INTO scores(user_id, game_id, points, timestamp) VALUES ($1, $2, $3, $4) RETURNING id`, [user_id, game_id, points, currentTimestamp]);
      ...
      socket.emit("leaderboard-update-from-backend", { response: "Success", gameID: game_id }); 
      return newScoreID;
    }),
  ...
  topPlayersByGame: publicProcedure
    .input(
      z.number()
    )
    .query(async (opts) => {
